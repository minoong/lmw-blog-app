...
/**
 * posts 폴더 내의 모든 카테고리 목록을 가져오는 함수 (중복 제거 포함)
 * - 변환 전 카테고리(`categoryRaw`)만 리스트로 반환
 */
export function getMdxCategories(): string[] {
 const categories = getMdxPostsInfo().map(({ categoryRaw }) => categoryRaw);
 return Array.from(new Set(categories));
}

/**
 * posts 폴더 내의 모든 카테고리를 변환된 타이틀 케이스로 반환하는 함수
 * - 변환 후 카테고리(`category`)만 리스트로 반환
 */
export function getFormattedCategories(): string[] {
 return getMdxPostsInfo()
